...
import type { SettingGroup } from '@vben/api'

import { computed, onMounted, reactive, ref } from 'vue'

import { settingUiApi } from '@vben/api'
import { Page } from '@vben/common-ui'
import { useIsMobile } from '@vben/hooks'
import { MdiRefresh } from '@vben/icons'
  ...
  try {
    ...
    for (const group of groups.value) {
      for (const info of group.settingInfos ?? []) {
        ...
        // 调试：如果原始名称包含前缀，记录转换过程
        if (rawName !== name) {
          console.warn('[Setting] Removed prefix from:', {
            rawName,
            cleanName: name,
          })
        }
    ...
    ElMessage.error($t('page.setting.messages.loadError'))
    // 控制台输出便于调试
...
async function saveChanges() {
  if (changedNames.value.size === 0) {
    ElMessage.info($t('page.setting.messages.saveNoChange'))
    return
  }
  ...
  // 调试日志：显示发送到API的数据
  console.warn('[Setting] Saving with prefix:', {
    originalNames: [...changedNames.value],
    inputWithPrefix: input,
    prefix: formNamePrefix,
  })
  ...
  try {
    ...
    changedNames.value.clear()
    ElMessage.success($t('page.setting.messages.saveSuccess'))
  } catch (error) {
...
// 取消重置相关逻辑：后续不再支持前端批量或单项重置

// 根据返回的 properties.Type 选择组件类型（按字母序声明 union）
...
// 渲染层将字符串安全转换为数字；无法解析时返回 undefined 以显示为空
...
// 安全获取自定义设置项的下拉选项（避免模板中直接访问 .options?.value 导致类型告警）
...
// 判断自定义类型（避免模板里直接深入 getCustomSetting）
...
<template>
  <Page content-class="flex h-[calc(100vh-120px)] flex-col overflow-hidden">
    <!-- 顶部操作栏 - 固定不滚动 -->
    <div
      class="border-border bg-card flex-shrink-0 rounded-xl border px-6 py-4 shadow-sm"
    >
      <div class="flex flex-wrap items-center gap-2">
        <span class="text-lg font-medium">{{ $t('page.setting.title') }}</span>
        <div class="flex-1"></div>
        <el-button :loading="loading" :disabled="submitting" @click="load">
          <template #icon>
            <MdiRefresh />
          </template>
          {{ $t('page.setting.actions.refresh') }}
        </el-button>
        <el-button
          type="primary"
          :loading="submitting"
    ...
    <!-- 主要内容区域 - 可滚动 -->
    <div class="mt-4 flex min-h-0 flex-1 gap-4 overflow-hidden">
      <!-- 左侧导航 - 固定宽度，不滚动 -->
      <div class="w-80 flex-shrink-0">
        <el-card shadow="never" class="h-full p-4">
          <div
          ...
          <div v-else class="space-y-1" v-loading="loading">
            <div
            ...
            >
              <el-button
                :type="idx === activeGroupIndex ? 'primary' : 'default'"
            ...
            >
              ...
              <div shadow="never" class="border-border p-4">
                <el-form
                  ...
                  class="space-y-4"
                >
                  <template v-for="info in group.infos" :key="info.name">
                    <el-form-item :label="info.displayName || info.name">
                      <!-- 说明文字显示在控件上方 -->
                        ...
                        <div class="flex w-full items-center gap-2">
                          ...
                          <template v-if="isCustomSetting(info.name ?? '')">
                            ...
                            <el-select
                              ...
                              fit-input-width
                              @change="markChanged(getSettingName(info))"
                            >
                              <el-option
                                v-for="option in customOptionsOf(info)"
                                :key="option.value"
                                :label="option.label"
                                :value="option.value"
                              />
