...
import type { MediumDto } from '../typing'
import { createBaseCurdUrl } from './curd-api-definition-base'
// 列表/详情通用基础输出（含审计字段）
...
>(baseUrl: string, mediumType: MediumType) {
  return {
    async update(input: TUpdateInputEntity, id: TId) {
      ...
      ;(item as unknown as MediumDto).mediumType = mediumType
      return item
    },
    delete(id: TId) {
    ...
    async updateCover(id: string, cover: File) {
      ...
      ;(entity as unknown as MediumDto).mediumType = mediumType
