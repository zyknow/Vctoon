...
public class ZyknowLuceneApplicationModule : AbpModule
{
    ...
    public override void OnApplicationInitialization(Volo.Abp.ApplicationInitializationContext context)
    {
        ...
        if (!options.EnableAutoIndexingEvents) return;

        var eventBus = sp.GetService<ILocalEventBus>();
        if (eventBus == null) return; // 容器未提供本地事件总线时跳过
    ...
    private static void Subscribe(ILocalEventBus bus, System.Type eventType, object handler)
    {
        var method = typeof(ILocalEventBus).GetMethods()
            .First(m => m.Name == nameof(ILocalEventBus.Subscribe) && m.IsGenericMethod && m.GetParameters().Length == 1);
